...
class ArithmeticSequenceRange {
 public:
  ...
  constexpr ArithmeticSequenceRange(Index first, Index size, Index incr) : first__{first}, size_{size}, incr_{incr} {}		// Anboto
  constexpr Index operator[](Index i) const { return first() + i * incr(); }												// Anboto
  constexpr Index first() const noexcept { return first__.value(); }
  constexpr Index size() const noexcept { return size_.value(); }
  constexpr Index incr() const noexcept { return incr_.value(); }

 private:
  variable_if_dynamicindex<Index, int(FirstAtCompileTime)> first__;															// Anboto
